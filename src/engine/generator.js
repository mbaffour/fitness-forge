...
import { EXERCISES, CARDIO_STRUCTURES, getExercisesForGroup, rankedForGroup, stapleRank } from '../data/exercises.js';
...
// Pick one recognizable exercise for this muscle. The ranking (primary-group
// first, curated staples above the bulk libraries, no cardio/SMR) lives in
// exercises.js so Overload, Freestyle and Body Map all agree; the choice stays
// random *within* the best tier so sessions still vary between shuffles.
function pickForGroup(group, equip, level, used) {
  const all = rankedForGroup(getExercisesForGroup(group, equip, level), group);
  let pool = all.filter(e => !used.has(e.id));
  if (!pool.length) pool = all;
  if (!pool.length) return null;

  const best = stapleRank(pool[0], group);
  const top  = pool.filter(e => stapleRank(e, group) === best);

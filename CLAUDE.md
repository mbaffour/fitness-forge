...
Exercise artwork (the workout-guide illustrations are pure-white silhouettes,
`fill:#fff`, no strokes) is theme-linked through three tokens: `--anim-bg` (the
surface behind it), `--anim-label` and `--anim-invert`. Dark themes keep the
white figure on a dark well (`--anim-invert: 0`); the light themes `day` and
`vivid` set `--anim-invert: 1`, flipping it to dark ink on a warm card. Only
workout-guide art carries the filter — the Gym Visual GIFs and free-exercise-db
stills are already dark-on-light and must never be inverted.
...
| `v3.23` | **Verified by running the app, not by reading it — and it found real bugs.** Every prior check proved only "no crash" + "HTTP 200"; the sandbox blocks the media hosts *for the browser*, so nothing had ever been seen to render. Proxying those hosts into headless Chromium gave real evidence: **1057/1057 exercise visuals decode** (`naturalWidth > 0`), the 3-frame animation genuinely cycles, thumbnails paint on all 11 surfaces, a logged session produces exactly the recomputed volume/PR/streak numbers (warm-ups excluded, kg→lbs correct), 21 pages render clean at desktop **and** 390px with no overflow, and the app boots + logs a workout with the network cut. Three defects fixed: (1) **the artwork was near-invisible** — workout-guide illustrations are pure-white silhouettes (`fill:#fff`, no strokes) drawn on a cream `#f3efe7` surface; the treatment is now **theme-linked** via `--anim-bg` / `--anim-invert` — dark themes put the white figure on a dark well, `day` and `vivid` invert it to dark ink on a warm card, and only workout-guide art carries the filter (the GIFs and photo stills are already dark-on-light). Applies to the modal, the tutorial phase strip and every list thumbnail; (2) **body-map muscles had no outlines** — a 724×1448 viewBox rendered into ~200px shrank `stroke-width:1` to a fraction of a pixel, so a cold-start figure read as an empty silhouette (now `vector-effect="non-scaling-stroke"`, untrained fill from `--bg-4`); (3) **finishing a workout left stale numbers** — `closeOverlay()` restored the page without re-rendering, so the dashboard still showed `STREAK 0 / SESSIONS 0` (new `window.refreshCurrentPage()`). **Videos: 65 → 1002 of 1057 (95%), and they play in the app.** The modal no longer hides the player behind "CLICK TO LOAD VIDEO" — the youtube-nocookie iframe embeds inline with a link out for clips whose owner disabled embedding. New `tools/resolve-youtube.mjs` resolves ids from YouTube's **public search results page** — no API key, no quota (the Data API's 100-units-per-search cap is why the key-based importer never ran) — accepting a result only when the title matches the movement and does not contradict its equipment; 54 exercises failed that gate and keep the search card rather than getting a wrong video. `tools/verify-videos.mjs` then checked all 1003 via oEmbed: **1002 alive and consistent, 1 dead cleared**, and it is now hardened so only a definitive HTTP status can condemn an id — a network error is reported as *unreachable* and never acted on. |
| `v3.24` | **Starting weights are computed from the person, not a lookup table.** The cold-start model multiplied a per-exercise bodyweight fraction by a *second* level fraction, halving every number, and fell back to a flat `0.25 x bodyweight` for any exercise not in its 60-entry table — 950 of the 1057 in the library. A 185 lb intermediate was told to bench 60, squat 90, and to load 25 lbs on Bird Dog, Board Press, Belt Squat and Band Lat Pulldown alike. That is the generic-weight bug. Replaced with a real model in `engine/overload.js`: an approximate **1RM strength standard per movement pattern** (26 patterns, resolved by curated id → name keywords → muscle group + compound/isolation, so every exercise lands somewhere sensible), scaled by **bodyweight** (allometric, not linear), **training level**, **sex** (upper-body standards differ more than lower; unstated takes the conservative middle) and **age** (peak to 35, then a taper; under-18 held well back), converted to a working load with inverse Epley and cut a further 15% (22% for beginners) because it is a first set. **Health guardrails:** a per-pattern ceiling as a multiple of bodyweight, the empty bar as the floor for big barbell lifts (an accessory exemption so a 140 lb beginner is not handed a 45 lb barbell curl), plate-real rounding, and `null` rather than a fabricated number for bands, cardio machines and bodyweight movements. Bodyweight detection is now **data-driven** (`requires: []`) instead of an 18-id list, so the 416 unloaded movements say "chase clean reps" — Bird Dog no longer quotes a barbell load. Dumbbell and kettlebell loads are **per implement**, with single-implement movements (goblet, single-arm, swings) not halved. Every suggestion now **states its basis** ("Start estimate from 185 lb bodyweight · intermediate · age 30 — deliberately light"), and a profile with no bodyweight says so instead of silently showing nothing. Pattern matching bugs found by sweeping all 1057: *Close-Grip* anything was filed as grip training (8 lb for a close-grip DB press), a barbell hack squat inherited the leg-press standard (320 lb), `Pin Presses` never matched `\bpress\b`, and plyometrics were loaded like their strength versions. **Same section, second defect:** the Overload rotation picked its exercises *uniformly at random* from the several-hundred-entry pool per muscle, so it served up **Jogging, Treadmill** for quads and **Bottoms-Up Clean From The Hang Position** for biceps. `pickForGroup` now excludes cardio/SMR/mobility entries and ranks curated staples above `wg_` above `fx_`, randomising only within the best tier — and ranks *before* preferring compounds, which had been discarding every curated core exercise (plank, hanging leg raise and ab wheel are all typed isolation) and leaving the core slot to conditioning moves. |
| `v3.25` | **Freestyle and Body Map get the same treatment — and one shared rule replaces three disagreeing ones.** Freestyle carried its **own duplicate copy of the cold-start weight model**, with both original faults intact: a level fraction multiplied on top of a per-exercise bodyweight fraction, and a flat `0.25 x bodyweight` fallback outside its 20-entry table. Because it was a second model it also **ignored your training history entirely** — a lift you had logged for months still quoted a first-timer's load. Deleted; Freestyle now asks `suggestNextSet` like everything else, so it reads history, states its basis, and marks per-hand dumbbell loads. **Exercise selection** is now one shared ranking in `exercises.js` (`isStrengthExercise` / `stapleRank` / `rankedForGroup`) used by Overload, Freestyle and Body Map, which each had their own rule and disagreed: Freestyle sorted only on compound-vs-isolation, so a **biceps** session opened with *Pull-Ups · Chin-Ups · Lat Pulldown* and **core** with *Farmer Carries · 3/4 Sit-Up · Air Bike*. The shared rank puts movements whose **primary** target is that muscle first, curated staples above `wg_` above `fx_`, bands and bodyweight last within a tier (alphabetical sorting had put *Band Chest Press* ahead of *Barbell Bench Press*), and drops cardio / stretch / mobility / SMR entries everywhere rather than only in Overload. Two more engine bugs surfaced by wiring Freestyle up: cold start **hard-coded 8 reps**, so switching Freestyle to *heavy* (4–6) quoted the same weight as *light* (12–15) — it now uses the actual rep target, and bench reads 110 / 120 / 135 across the three; and an exercise whose only equipment is a **tool with no adjustable load** (`ab_wheel`, `rings_trx`, joining bench and pull-up bar) was given a weight, so Ab Wheel Rollout asked for 35 lbs. Bodyweight movements 416 → **418**. |
| `v3.26` | **Equipment and Calisthenics finish the sweep.** Equipment already routed through `buildFullBodyOverload` + `suggestNextSet`, so it inherited the v3.24–25 fixes intact — but its **coverage grid counted cardio machines, stretches and SMR entries**, overstating a thin kit by ~104 exercises (home-basic 1439 → 1335 honest, full-gym 2545 → 2415). Now filtered through `isStrengthExercise`. **Calisthenics** had the bodyweight version of the generic-weight bug: the rep target was a **fixed string per exercise**, so the page told everyone the same thing forever no matter what they had logged. Bodyweight progression *is* rep progression, so the base range now goes through the same engine — a logged Push-Up moves 8 → *9 reps · push for one more*, 12 → *13*, and 20+ → *22 reps ✓ · try adding weight or a harder variant*. `suggestNextSet` gained `signal` + `bestReps` on the bodyweight branch so a caller can tell "topped the range" from "add a rep" without parsing the rationale: on a hit-upper the engine resets reps to `targetMin` (correct when you are about to add load) which read as a demotion on a reps-only page, so Calisthenics holds the mark you actually reached. Also removed a dead `getCalisthenicsExercises` import (the page builds from curated category lists). **Two engine bugs found on the way:** `parseRepRange` split on `-` after stripping a trailing `s`, so **every suffixed range collapsed to `min === max`** — `"10–20 reps"` parsed as 10–10 and `"8–12/side"` as 8–8, misfiring the rep-progression signal wherever a range carried a unit; it now reads the numbers directly. And the shared `isStrengthExercise` filter matched `\bwalking\b` anywhere, so **Dumbbell / Barbell / Bodyweight Walking Lunge were being excluded as cardio** — gait words now only match when the whole name is the activity (*Walking, Treadmill*), keeping the lunges. Excluded entries 76 → **74**. |
| `v3.27` | **Programs and Challenges — the last two sections.** A Program session preview listed `4 × 8-10` and nothing else, so the one thing you need at the rack — **what to put on the bar** — was the only thing missing. Every row now carries the load from the shared engine (`Barbell Bench Press 5 × 5 · 130 lbs`, `Incline DB Press 4 × 8-12 · 35 lbs/hand`, `Dips 3 × 8-10 · BW`), per-hand marked and history-aware like everywhere else. **Challenges were one fixed curve for everyone** — day 1 of the 30-Day Push-Up asked 11 reps whether you can do 5 or 40: impossible for one person, trivial for weeks for the other. The authored curve is now **scaled to your best logged set** for that movement, anchored on the **finish** rather than day 1: the authored ramps grow ~5× over 30 days, so scaling by day 1 preserved that multiple and sent someone already doing 40 push-ups to a **134-rep set**. Aiming the last day at ~2.2× current best gives a hard but real finish, with day 1 landing near half of what you can do today — best 6 → *4 … 22*, best 12 → *5 … 26*, best 40 → *16 … 88*. Works for timed holds too (90s plank → *46s … 198s*). Scale is clamped to 0.4–3× so one freak set cannot warp the ramp, and with no history the authored numbers stand with the card saying so ("Default ramp. Log Push-Up once and it re-anchors"). The two habit challenges with no `exId` (Daily Movement, Daily Mobility) correctly show no basis line — there is nothing to anchor to. |
| `v3.28` | **Stress testing + tag backfill.** Scale: a 400-session / 1.1 MB history renders all 21 pages in ~536 ms total, second render 9 ms — the one apparent outlier (nutrition 268 ms) was first-time Chart.js init, not an O(n) scan. Churn: 200 navigations + 60 modal cycles + 40 body-map toggles + 25 session start/cancels leave node count, canvas count and heap **flat**, 0 errors, 0 stray overlays, and intervals returning to 0 (2 live during rest, 0 after cancelling mid-rest, tab title restored). Two real defects fixed. **(1) `save()` swallowed every failure** (`try { … } catch {}`), so in Safari private mode or with site data blocked the app kept showing "✓ SAVED TO YOUR LOG" while nothing persisted. It now returns a boolean, dispatches `forge:save-failed` / `forge:save-recovered` (no UI import in `store.js`), raises a pinned banner telling you to export a backup, and the session summary reads **"⚠ NOT SAVED"** instead of claiming success; the banner clears itself when a write next succeeds. **(2) A restored/legacy backup blanked the Progress page** — `profile.equipment.replace()` was unguarded, so a profile without that field threw before anything reached the DOM, leaving a blank page and no on-screen error. Nine hostile saved states (empty, null program+profile, v1-era shape, arrays-as-objects, missing set fields, absurd numbers, NaN/Infinity, deep nulls, corrupt JSON) now all boot and navigate every page cleanly. **Correction to an earlier claim in this sweep:** an initial "session lost on reload" result was a *test-harness artifact* — the Playwright harness re-seeded `localStorage` on every navigation, wiping what the app had just written. Persistence verified properly afterwards (log → reload → session, volume and streak all intact). Known bound, unchanged: `logSession` keeps 200 sessions of per-set detail; `workoutLog` keeps 500 summaries and `prs` is stored separately, so beyond ~200 sessions only per-set detail ages out. **`tools/backfill-tags.sh`** recreates the 28 release tags that were never pushed (v2.9-stable, v3.0-stable, v3.3–v3.27), each pointed at the commit that shipped it; idempotent, and `--push` sends whatever is local-but-missing-on-remote. |
| `v3.29` | **More stress tests, four more fixes — and the app now has a feel.** Fuzzing the logger with 12 hostile inputs found that a number input is not validation: **`-500` stored as negative volume**, **`1e308` made session volume `Infinity`** — which `JSON.stringify` writes as `null`, permanently erasing that session's tonnage — and it **registered a 1e308 PR that nothing could ever beat**, freezing progression for that lift forever. Shared `clampWeight` / `clampReps` / `clampSeconds` in `store.js` now gate every path that records a set (logger, edit-set, CSV import); a typed negative **re-prompts** rather than silently logging a loaded lift as bodyweight; `computeSessionVolume` and `estimateOneRepMax` refuse to return non-finite values; and PR detection ignores sets above 30 reps, where Epley stops meaning anything. **Router fix:** `PAGES['__proto__']` and `PAGES['constructor']` are truthy on a plain object, so a crafted `#__proto__` hash walked past every guard, deactivated every page and rendered nothing — **a blank app with no error**. Membership is now an own-property test (`cue()` got the same treatment). Passing: triple-tapping Finish still writes one session; streak maths correct across yesterday / today / gap / future dates. **Feel:** `feedback.js` gains six cues — `select`, `warmup`, `error`, `start`, `milestone`, `achievement` — with haptic patterns shaped to be told apart in a pocket (a warm-up `[12]` is lighter than a working set, an error is one blunt `[70,40,70]`, a milestone climbs). New `countUp()` animates session volume rather than snapping it, `sparkBurst()` throws forge sparks off the anvil when a set lands, the dashboard flame **flares** the first render after your streak grows, a claimed challenge day gets **stamped**, a tapped muscle **pulses** before its list renders, primary buttons take a strike on press, and a rejected input shakes red in time with the error buzz. All of it is decorative, so all of it is switched off under `prefers-reduced-motion` — verified, along with every cue firing, in the browser. |
| `v3.30` | **XSS, and a malformed log that blanked whole pages.** Every page builds markup with template literals and assigns it via `innerHTML` — ~93 interpolation sites — and user strings reached them unescaped. A profile name of `<img src=x onerror=…>` **executed on every page that greets you by name**; the same held for gym-profile names, custom-exercise names and cues, session labels and notes. **4 payloads fired** in the test sweep. This is not only self-inflicted: the app imports JSON backups and third-party CSV (Strong / Hevy / FitNotes), so a tampered file would run script in this origin with access to every workout, measurement and note stored. Rather than escape 93 output sites — one missed addition reopens the hole — free text is cleaned once at the **few doors it enters through** (`cleanText` in `store.js`, applied in `setProfile`, `saveGymProfile`, `addCustomExercise`, `logSession`, `logWorkout`, `addBodyCheckIn`, `addSleepEntry`, `addActivityEntry`): angle brackets cannot form a tag, double quotes cannot close an attribute, and apostrophes and accents survive intact. A **boot-time scrub** cleans text already in storage, so a payload that arrived in an earlier backup stops firing. Now **0 executions, 0 injected tags, 0 stray handlers**. **Second bug, worse than it looked:** a `nutritionLog` saved as an object (or holding a `null` day) threw `.find is not a function` **during shell construction** — so `#page-nutrition` and everything after it were never created. Not a blank page: a blank app, with no error on screen. The state loader filled in *missing* keys but never type-checked ones that were present, so every collection is now coerced to an array (nulls dropped) or object at load, and `getTodayNutrition()` normalises a day it *found* rather than only one it created. Four malformed-nutrition shapes now all render. |
